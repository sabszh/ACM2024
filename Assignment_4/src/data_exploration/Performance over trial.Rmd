...
```{r}
# setwd("./Assignment_4/src/data_exploration")
...
# Load simulated fitted model
sim_fit <- readRDS(file = "../stan/simulated_fit.rds")
# Load empirical fitted model
emp_fit <- readRDS(file = "../stan/empirical_fit.rds")
...
sim_draws <- as_draws_df(sim_fit$draws())

# Get posterior means for c and w
mean_c <- mean(sim_draws$c)
mean_w <- colMeans(sim_draws %>% select(starts_with("w")))
